...
![HA](https://img.shields.io/badge/High_Availability-success?style=for-the-badge)
...
- Why is GLBP's priority logic the same as HSRP (higher = AVG), and how did we make DIST-2 the active forwarder?
- Why configure the trunk on BOTH ends of an access↔distribution link?
- Why dual-home one DHCP server to both cores instead of running two servers with split-scope?
- Why does each distribution switch relay to a different DHCP server?
- Why L3 routed links between distribution and core instead of trunks?
- Why align STP root with the FHRP active gateway?
- How does IP SLA + tracking create internet failover, and how do the two cores back each other up?
- Why does NAT fail on a Layer 2 "core," and why must it live on a Layer 3 router (the ISP edge)?
- Why list each VLAN in the NAT ACL instead of 192.168.0.0/16 when the DHCP uplink shares that range?
...
**Designed in layers. Routed with OSPF. Redundant everywhere.**
